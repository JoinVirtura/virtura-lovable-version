...
export const ContentCard = ({ tile, className = "", size = 'md' }: ContentCardProps) => {
  return (
    <Card className={`group relative overflow-hidden border-0 bg-transparent hover:scale-[1.01] transition-all duration-300 ${className}`}>
      {/* Image Container */}
      <div className="relative aspect-square overflow-hidden rounded-xl bg-gradient-to-br from-card to-card/50">
        <img
          src={tile.posterUrl}
          alt={tile.title}
        ...
        {/* Bottom Content */}
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent p-4 translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
          {/* Title */}
          ...
          <div className="flex items-center justify-between text-white/80 text-sm">
            <div className="flex items-center gap-1">
              ...
              <span>{tile.views ? `${tile.views.toLocaleString()}` : '0'}</span>
            </div>
            {tile.duration && (
              <div className="flex items-center gap-1">
