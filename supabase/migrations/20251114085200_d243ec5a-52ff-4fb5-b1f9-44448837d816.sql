...
-- A/B test metrics
CREATE TABLE notification_ab_metrics (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  test_id UUID NOT NULL REFERENCES notification_ab_tests(id) ON DELETE CASCADE,
  variant_id UUID NOT NULL REFERENCES notification_ab_variants(id) ON DELETE CASCADE,
  user_id UUID NOT NULL,
...
CREATE TABLE notification_sms_log (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL,
  notification_id UUID REFERENCES notifications(id) ON DELETE SET NULL,
...
-- SMS rate limiting
CREATE TABLE notification_sms_limits (
...
-- Aggregated CTR metrics (materialized view)
...
CREATE INDEX idx_ab_metrics_test_variant ON notification_ab_metrics(test_id, variant_id);
CREATE INDEX idx_ab_metrics_clicked ON notification_ab_metrics(notification_clicked);

-- SMS indexes
CREATE INDEX idx_sms_log_user_id ON notification_sms_log(user_id);
CREATE INDEX idx_sms_log_status ON notification_sms_log(status);
...
CREATE INDEX idx_analytics_user_id ON notification_analytics(user_id);
CREATE INDEX idx_analytics_event_type ON notification_analytics(event_type);
CREATE INDEX idx_analytics_timestamp ON notification_analytics(event_timestamp);
CREATE INDEX idx_ctr_metrics_date ON notification_ctr_metrics(date);
CREATE INDEX idx_ctr_metrics_category ON notification_ctr_metrics(category);
